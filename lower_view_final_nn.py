...
def lower_onetwo_nn(shot, time, onetwo_save_path,
    ...
    支持两种模式：
      1. NN 模式（推荐）：传入 te_profile_nn + ne_profile_nn，跳过 mtanh 拟合
      2. 传统模式（fallback）：不传 NN 剖面，使用 mtanh 拟合（与原版相同）
    ...
    参数:
        ...
        onetwo_save_path : str
            输出保存路径
        te_profile_nn : ndarray or None
            NN 拟合的 Te 剖面 [keV]（可选，提供则跳过 mtanh 拟合）
        ne_profile_nn : ndarray or None
            NN 拟合的 ne 剖面 [1e19 m^-3]（可选）
        te_rho_nn : ndarray or None
            Te 剖面对应的 rho 网格（与 te_profile_nn 等长）
        ne_rho_nn : ndarray or None
            ne 剖面对应的 rho 网格
