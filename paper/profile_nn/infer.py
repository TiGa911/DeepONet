"""ProfileNet 推理模块：替代 mtanh + 样条拟合法链的即插即用函数。
...
使用示例：
    ...
    set_model_dir('./profile_nn_models')  # 可选：指定模型目录

    rho_201, te_201 = nn_fit_te(rho_ts, te_ev)         # Te: eV 输入, keV 输出
    rho_201, ne_201 = nn_fit_ne(rho_refl, ne_1e19)     # ne: 1e19 m^-3 输入输出
    rho_201, ti_201 = nn_fit_ti(rho_txcs, ti_keV,      # Ti: keV 输入输出
                                te_ped_x, te_ped_y)
"""
...
def _load_model(datatype, model_dir=None):
    """从磁盘加载训练好的 ProfileNet 模型。按诊断类型缓存在进程内存中。

    加载流程：
      1. 检查缓存 —— 同一进程内多次调用只加载一次
      2. 读取 .pt 检查点文件，提取模型配置（hidden 维度等）
      3. 实例化对应架构类（ProfileNet_Te/ne/Ti）
      4. 加载权重并设为评估模式（eval）
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            f"模型未找到: {model_path}\n"
            f"请先训练模型: python -m profile_nn.train --data <data_dir> --datatype {datatype}"
        )
    ...
    model.eval()  # 评估模式：关闭 dropout、batch norm 等训练行为
...
def nn_fit_te(rho_scattered, te_scattered, model_dir=None):
    ...
    # 单位归一化：eV → keV（匹配 mtanh 拟合器的内部约定和训练数据分布）
    te_keV = te / 1000.0
    ...
    # 这是在推理端的最后防线；训练时的 PhysicsConstrainedLoss 已大幅减少需要修正的情况
    try:
        ...
        pass  # PCHIP 失败时退回到原始 NN 输出

    # 边缘下界保护：防止物理上不可能的近零值导致下游 ONETWO 输运求解器除零
    te_201 = np.maximum(te_201, 0.005)  # 5 eV — 远低于任何 LCFS 内部等离子体温度
    te_201 = np.minimum.accumulate(te_201)  # 重新施加单调性

    return rho_201, te_201


def nn_fit_ne(rho_scattered, ne_scattered, model_dir=None):
    """神经网络替代 ne 拟合法：Refl 散点 → ne 连续剖面。
    ...
    # 边缘下界保护：防止物理上不可能的近零值导致下游 ONETWO 输运求解器除零
    ne_201 = np.maximum(ne_201, 0.001)  # 0.001 × 10^19 m^-3 — 远低于边界密度
    ne_201 = np.minimum.accumulate(ne_201)  # 重新施加单调性

    return rho_201, ne_201


def nn_fit_ti(rho_scattered, ti_scattered, te_ped_x, te_ped_y, model_dir=None):
    """神经网络替代 Ti 拟合法：TXCS 散点 + Te 台基 → Ti 连续剖面。
    ...
    # Ti 模型有双输入，不能直接用 _predict_profile 辅助函数
    ...
    # 边缘下界保护：防止物理上不可能的近零值导致下游 ONETWO 输运求解器除零
    ti_201 = np.maximum(ti_201, 0.005)  # 5 eV — 远低于 LCFS 内部离子温度
    ti_201 = np.minimum.accumulate(ti_201)  # 重新施加单调性

    return rho_201, ti_201


def set_model_dir(path):
    """设置包含训练好的 .pt 模型文件的目录路径。

    调用此函数后：
      - 后续的 nn_fit_te/ne/ti 调用将从新路径加载模型
      - 缓存被清空，下一次调用会重新加载模型
