# -*- coding: utf-8 -*-
"""
onetwo_output_transformer.py — 基于 Transformer 的 ONETWO 输运计算流程

与 onetwo_output.py 的唯一区别：用 Transformer 神经网络推理替代了 mtanh+spline 拟合链。
...
def process_time_point(args):
    ...
    2. 用 Transformer 分别拟合 Te、ne、Ti 剖面
    ...
    if status['TS_status'] == 1:
        try:
            ...
            x_te_201, y_te_201 = nn_fit_te_transformer(x, y)
            ...
            plt.title(f'Shot {shot} @ {real_time:.3f}s [TF]')
            plt.xlim([0, 1])
            plt.legend()

            save_path = os.path.join(output_dir, f'Te_shot{shot}_time{time_dir}_tf.png')
            plt.savefig(save_path, dpi=150, bbox_inches='tight')
            plt.close()
            print(f'(TF_Te) success at {real_time:.3f}s -> {save_path}')

            # 写入 ONETWO 的 NAMELIS1：TEIN 需要 eV 单位
            obj['NAMELIS1']['RTEIN'] = list(x_te_201)
            obj['NAMELIS1']['TEIN'] = list(y_te_201 * 1e3)  # keV -> eV
        except Exception as e:
            _log_error(shot, time, 'TF_Te', str(e))
            TE_STATUS = 1
    else:
    ...
    if status['Refl_status'] == 1:
        try:
            ...
            x_ne_201, y_ne_201 = nn_fit_ne_transformer(x, y)
            ...
            save_path = os.path.join(output_dir, f'ne_shot{shot}_time{time_dir}_tf.png')
            plt.savefig(save_path, dpi=150, bbox_inches='tight')
            plt.close()
            print(f'(TF_ne) success at {real_time:.3f}s -> {save_path}')
            ...
            _log_error(shot, time, 'TF_ne', str(e))
            NE_STATUS = 1
    else:
    ...
    if status['TXCS_status'] == 1 and te_ped_x is not None and te_ped_y is not None:
        try:
            ...
            x_201, y_201 = nn_fit_ti_transformer(x, y, te_ped_x, te_ped_y)
            ...
            save_path = os.path.join(output_dir, f'Ti_shot{shot}_time{time_dir}_tf.png')
            plt.savefig(save_path, dpi=150, bbox_inches='tight')
            plt.close()
            print(f'(TF_Ti) success at {real_time:.3f}s -> {save_path}')

            # 写入 ONETWO 的 NAMELIS1：TIIN 直接用 keV，无需单位转换
            obj['NAMELIS1']['RTIIN'] = list(x_201)
            obj['NAMELIS1']['TIIN'] = list(y_201)
        except Exception as e:
            _log_error(shot, time, 'TF_Ti', str(e))
    elif status['TXCS_status'] == 1:
        print(f'(TF_Ti) Skipped at {time}s: No Te pedestal data available')
    ...
    if (TE_STATUS == 0 and NE_STATUS == 0):
        try:
            onetwo_result_dir = os.path.join(output_base, f"{shot}", time_dir, 'onetwo_transformer')
            ...
            print(f'(TF_onetwo) success at {time}s')

        except Exception as Error_onetwo:
            _log_error(shot, time, 'TF_ONETWO', str(Error_onetwo), exc_info=True)
