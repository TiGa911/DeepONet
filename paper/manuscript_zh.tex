...
\caption{典型 H 模剖面拟合结果：六种方法（mtanh 基线 + 五种 NN 架构）在三个代表性 H 模时间点上对 Te、ne、Ti 三种诊断的对比。灰色阴影带为验收区间（见第 3 节）：其半宽随诊断通道围绕 mtanh 参考的局部分散度变化，位于区间内的曲线视为可接受。}
\label{fig:hmode}
\end{figure}

在图~\ref{fig:hmode} 中，好的拟合可以直接判读：剖面应穿过芯部诊断通道、复现 mtanh 基线的整体形状，更重要的是在 $\rho = 0.85$--$1.0$ 处跟踪陡峭的台基梯度（H 模物理信息集中之处）；剖面应保持光滑、正值、单调，边缘区域无非物理振荡，并尽可能停留在诊断通道所约束的灰色验收区间内。过度平滑台基、在边缘振荡或明显出带的架构，在表~\ref{tab:metrics} 的 MAE 指标和表~\ref{tab:band} 的区间内比例上相应地更差。
...
\textbf{验收区间准则。}为使图~\ref{fig:hmode} 的定性对比获得定量解读，我们引入以 mtanh 参考为中心的数据驱动验收区间：每个时间点的区间半宽取诊断通道 $|y_{\text{散点}} - y_{\text{mtanh}}|$ 的局部分箱中位数（按 $\rho$ 分箱，下限 0.05 个剖面单位），因此区间代表诊断围绕参考的固有分散度，而非任意设定的容差。拟合曲线位于区间之内的部分即视为可接受。表~\ref{tab:band} 报告全部测试点的平均区间内比例。该评价复现了表~\ref{tab:metrics} 的 MAE 排名——LSTM 和 Transformer 在 Te、Ti 上领先，Transformer 在 $n_e$ 上的弱点一目了然（区间内仅 11\%）——同时给出可解释、逐剖面适用的验收准则，可在无需人工检查的批量处理中对每一炮自动判定。图~\ref{fig:hmode} 所示的代表性时间点还经过自动质量筛查（非有限值、非单调尖峰、大幅出带偏离），确保图中不出现明显异常的曲线。
