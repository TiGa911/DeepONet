...
\caption{典型 H 模剖面拟合结果：六种方法（mtanh 基线 + 五种 NN 架构）在三个代表性 H 模时间点上对 Te、ne、Ti 三种诊断的对比。灰色阴影带为数据驱动的验收区间，定义见第 3 节。每炮贡献其最接近 H 模 $H_{98}$ 中位数的时间点（此处 0.96--1.13），并经过第 3 节所述自动质量筛查——这些是测试集中条件最苛刻的 H 模情形。}
\label{fig:hmode}
\end{figure}

在图~\ref{fig:hmode} 中，好的拟合可以直接判读：剖面应穿过芯部诊断通道、跟踪 $\rho = 0.85$--$1.0$ 处陡峭的台基梯度（H 模物理信息集中之处），保持光滑、正值、单调，并停留在灰色验收区间内。这些判读准则分别由表~\ref{tab:metrics} 的 MAE 和表~\ref{tab:band} 的区间内比例定量化。
...
所有模型使用 AdamW 优化器（学习率 $10^{-3}$，权重衰减 $10^{-5}$）、余弦退火学习率调度、批次大小 64、最大 500 轮训练和早停机制（耐心值 = 100）。推理后应用 PCHIP（分段三次 Hermite 插值）保形插值，强制严格单调递减和边缘地板值保护。所有报告的指标均在后处理之后的剖面上计算；我们在 Te 测试点上验证过，PCHIP 步骤对散点拟合 MAE 和相对 mtanh 的 MAE 的影响均小于 0.001 keV，即不会实质改变报告数值。推理时间在 CPU 上测量（10 次预热后对完整推理函数计时 200 次，不含模型加载）；与 mtanh 流程的端到端速度对比见第 3 节。
...
图~\ref{fig:hl} 对比了各诊断-方法组合在 H 模和 L 模下的 MAE。神经网络在两种等离子体模式下表现一致，LSTM 和 Transformer 的 H/L 准确度最为均衡。L 模组现在包含低 $T_e$ 放电 156900（芯部 $T_e$ 低至 0.2 keV），将对比范围扩展到了比原四炮更冷的 L 模条件。图~\ref{fig:lmode} 给出对应的 L 模剖面叠加：模型紧密跟踪较平滑的 L 模剖面，与表~\ref{tab:metrics} 中更低的 L 模 MAE 一致。
...
\caption{典型 L 模剖面拟合结果：六种方法在三个代表性 L 模时间点上对 Te、ne、Ti 三种诊断的对比，验收区间阴影与图~\ref{fig:hmode} 同口径。}
\label{fig:lmode}
\end{figure}
...
消融结果呈现三种行为。基于池化和注意力的 DeepONet 在移除物理约束后略有改善（ProfileNet $-11.8\%$、Transformer $-8.7\%$）；没有分支-主干结构的 CNN-1D 退化 $+25.8\%$；而共享同一主干的 LSTM 退化更严重（$+30.6\%$，H 模 MAE 从 0.525 升至 0.805 keV）。机理阐释留待第 4.1 节；直接的实际结论是：分支-主干结构对静态编码器已是足够的正则化（物理项对它们轻微过正则化），而循环编码器仍依赖显式约束。

\subsection{工程价值：速度、自动化与验收准则}

\textbf{推理速度与确定性。}五种架构在单核 CPU 上的推理时间均低于约 20 ms（ProfileNet $\sim$15 ms、LSTM $\sim$14 ms、CNN-1D $\sim$16 ms、CNN-DeepONet $\sim$16 ms、Transformer $\sim$19 ms），既适用于离线批量处理，也满足潜在实时应用需求。作为参照，我们在同一 CPU 上对 24 个测试点计时了 mtanh 流程端到端运行时间：每个 Te 剖面 16--71 ms（中位数 36 ms），$n_e$ 为 16--870 ms（中位数 30 ms；最小二乘收敛缓慢造成长尾），$T_i$ 约 1 ms（其参数化拟合比 NN 更轻）。因此 NN 对 Te 比 mtanh 快 2--3 倍，在 $n_e$ 的慢收敛情形下更快，而对 $T_i$ 的原始时延反而慢于 mtanh。实际收益并非单纯的速度，而是统一性：一次确定性的 14--19 ms 前向传播，无逐例优化器失败模式、无需人工检查。诊断最优集成方案（CNN-based $T_e$ + ProfileNet-based $n_e$ + LSTM-based $T_i$，共 66K+67K+76K 参数）组合推理时间低于 50 ms。

\textbf{自动化与鲁棒性。}以单次前向传播取代七步流程，消除了主要的规模化瓶颈：目前每次 mtanh 拟合都必须人工目视检查，台基最小二乘优化偶尔失效或收敛到非物理解，需要人工识别排除。NN 流程无需此类检查——其输出在构造上即光滑、正值、经 PCHIP 后单调，即使输入超出训练分布也只会平缓失效而非灾难性失效。无效或缺失的诊断通道由与训练相同的掩码处理，无需修改代码分支；训练流程已完全脚本化，可随新实验数据的积累定期重训。这些性质对 EAST 每年度数千炮的吞吐量至关重要——人工检查正是当前的限速环节。

\textbf{验收区间准则。}为使图~\ref{fig:hmode} 的定性对比获得定量解读，我们引入以 mtanh 参考为中心的数据驱动验收区间：每个时间点的区间半宽取诊断通道 $|y_{\text{散点}} - y_{\text{mtanh}}|$ 的局部分箱中位数（按 $\rho$ 分箱，下限 0.05 个剖面单位），因此区间代表诊断围绕参考的固有分散度，而非任意设定的容差。拟合曲线位于区间之内的部分即视为可接受。表~\ref{tab:band} 报告全部测试点的平均区间内比例。该评价复现了表~\ref{tab:metrics} 的 MAE 排名——Transformer 在 Te 上领先、LSTM 在 $T_i$ 上领先，Transformer 在 $n_e$ 上的弱点一目了然（区间内仅 11\%）——同时给出可解释、逐剖面适用的验收准则，可在无需人工检查的批量处理中对每一炮自动判定。图~\ref{fig:hmode} 所示的代表性时间点还经过自动质量筛查（非有限值、非单调尖峰、大幅出带偏离），确保图中不出现明显异常的曲线。

\begin{table}[htbp]
\centering\footnotesize
\caption{验收区间评价：拟合剖面位于散点导出区间（围绕 mtanh 参考，见图~\ref{fig:hmode}）内的平均比例（\% $\pm$ 标准差），统计全部测试时间点。每行最优值用\textbf{粗体}标出。}
\label{tab:band}
\begin{tabular}{l c c c c c}
\toprule
\textbf{诊断} & \textbf{ProfileNet} & \textbf{LSTM} & \textbf{CNN-1D} & \textbf{CNN-DeepONet} & \textbf{Transformer} \\
\midrule
...
表~\ref{tab:metrics} 显示没有单一架构在三种诊断上均最优。我们将其归因于三种诊断的空间采样方式差异，而非模型容量差异。TS 提供 20--25 个密集均匀通道，CNN-1D 的局部卷积核天然利用这种局域性。Refl 在不规则、集中于边缘的 $\rho$ 网格上采样 $n_e$，ProfileNet 的排列不变集合池化最为鲁棒。TXCS 仅提供 5--15 个稀疏、不规则分布的点，BiLSTM 的门控序列建模最稳健。CNN-DeepONet 对照组直接隔离了解码器的贡献：对稀疏 TXCS 数据，将 CNN-1D 的卷积解码器换成共享主干使 $T_i$ MAE 降低 42\%；对密集 TS/Refl 数据主干无改善。密集诊断上的轻微变差（Te 0.342$\to$0.393 keV，$n_e$ 0.067$\to$0.075）表明，主干的 128 个基函数子空间叠加保正 Softplus 输出，会轻微过度平滑密集局部数据本可分辨的陡峭台基特征——这正是同一结构保证在稀疏输入上获益的代价。对 128 个已训练主干基函数的频谱分析定量佐证了这一点：所有基函数的主导波数均低于每单位 $\rho$ 3 个周期（中位数 1；图~\ref{fig:trunk}），即 DeepONet 输出空间无法表示比台基尺度更尖锐的特征——排除振荡的结构性光滑，正是密集数据中尖锐特征被轻微抹平的原因。主干的平滑作用因此对稀疏输入重要，在局部连续性已足够的场合冗余（且有轻微代价）。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{paper_results/figures/fig_trunk_spectrum.png}
\caption{共享 CoordDecoder 的空间频率内容：(a) 六个示例主干基函数 $b_k(\rho)$；(b) 全部 128 个基函数主导波数（FFT 峰值）的直方图。所有基函数的主导波数均 $\leq 3$ 周期/单位 $\rho$（中位数 1），定量刻画了 DeepONet 输出空间的结构性光滑。}
\label{fig:trunk}
\end{figure}

实践建议：部署诊断匹配的集成模型（CNN-based $T_e$ + ProfileNet-based $n_e$ + LSTM-based $T_i$，组合推理 <50 ms），在不要求单一通用架构的前提下取得各诊断的最佳精度。
...
\begin{enumerate}
    \item \textbf{拟合精度}：NN 方法相对 mtanh 的 MAE 为 $T_e = 0.34$--$0.51$ keV、$n_e = 0.06$--$0.11 \times 10^{19}$ m$^{-3}$、$T_i = 0.07$--$0.14$ keV，全部测试时间点的芯部峰值度相关系数高（$r > 0.95$）。
    ...
    \item \textbf{工程价值}：NN 流程消除了人工检查与逐例优化器失效，以统一确定性的 14--19 ms 时延运行（对 $T_e$ 比 mtanh 快 2--3 倍），通道缺失或无效时平缓降级，并配套数据驱动的验收区间准则（表~\ref{tab:band}）自动标记不可接受的拟合——为无需人工干预地批量处理每年度数千炮放电提供了实用基础。
...
\begin{table}[htbp]
\centering\footnotesize
\caption{NN 方法相对 mtanh 基线的平均相对误差（MeanRel\% $\pm$ 标准差），按诊断与等离子体模式统计（H 模：8 个时间点；L 模：10 个时间点）。MeanRel\% 由低值边缘区域（$\rho \to 1$）主导——该处微小绝对偏差即产生大相对误差。L 模 Te 行被新增的 156900 低 $T_e$ 点（芯部 $T_e$ 0.2--1.1 keV）抬高——其边缘值趋近于零、相对误差病态；原四炮的 L 模 Te 误差为 22--30\%，低于 H 模的 33--57\%。每行最优值用\textbf{粗体}标出。}
\label{tab:meanrel}
\resizebox{\textwidth}{!}{%
\begin{tabular}{c c | c c c c c}
\toprule
