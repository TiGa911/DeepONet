...
由 NN 拟合剖面计算的总驱动电流 $I_{\text{LH}}$ 与 mtanh 基线的平均绝对偏差 $\Delta I_{\text{LH}} / I_{\text{LH, mtanh}}$ 为 21.7\%（156400 炮），四个 LHW 测试炮合计为 27.7\%（各炮 11.8--53.4\%）。为归因该偏差，我们将各架构的 $T_e$ 或 $n_e$ 剖面逐一换回 mtanh 参考后重算 $I_{\text{LH}}$：在 156400 炮的 6 个时间点上平均，$T_e$ 差异贡献了 21.7 个百分点平均绝对偏差中的 20.7 个，$n_e$ 差异贡献 3.9 个，非线性交叉项贡献 1.5 个——LHW 电流偏差约以 5:1 的比例由 $T_e$ 主导，与模型中朗道阻尼共振和准线性扩散系数对 $T_e$ 的强依赖一致。各方法间的离散度很大（如该炮 CNN-1D 为 $-25\%$ 至 $-33\%$、CNN-DeepONet 为 $+34\%$ 至 $+55\%$），可追溯到第 3 节所述的台基区 $T_e$ 差异。

\textbf{重要说明}：该对比量化的是剖面差异经 LHW 模型的\textit{传播}，而非哪种方法产生"正确"的 LHW 预测。缺乏直接对标的实验电流驱动测量验证。此外，ONETWO 输运计算（可提供更完整的物理验证）因 ONETWO Fortran 程序中 ZFIT 原子物理温度范围限制（退出码 91）未能完成，这是已知的 EAST 与 CFETR 设计模板之间的兼容性问题。
...
作者感谢 EAST 团队提供诊断数据与运行支持。本工作由 [基金信息待补] 资助。
